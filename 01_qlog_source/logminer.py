...
def find_taper_i(m):
    ...
    for i in range(len(ls)):
        if ls[i] ==  ls[i+1]- 1 == ls[i+2] - 2 == ls[i+3] -3 == ls[i+4] - 4:
            return ls[i]
    return -1
...
def process_state(m):
    ...
    #总体描述
    #1.充电类型
    ax.text(0.1,0.47,'Charger type: ' + ctype,
    horizontalalignment='left', verticalalignment='top', fontsize = 15, color ='b',alpha = 1)
    
    #2. 充电 soc time过程:
    if m.is_soc_100 == False:
        soc_text = str(df['SOC'].min()) + '%' +' ~ ' + str( df['SOC'].max()) + '%'
        time_text = str(df['Uptime'].min()) + 's' + ' ~ ' + str( df['Uptime'].max() ) + 's'
    else:
        mid_text = str( df['SOC'][m.i_taper] )  +'%' +'(taper) ~ ' 
        soc_text = str(df['SOC'].min()) + '%' + ' ~ ' + mid_text +str( df['SOC'].max()) + '%'
        ...
        time_text = str(df['Uptime'].min()) +'s' + ' ~ ' + mid_text + str( df['Uptime'].max() ) + 's' + t_full_str
    
    
    ax.text(0.1,0.4,'Charging SOC form: ' + soc_text, 
    horizontalalignment='left', verticalalignment='top', fontsize = 10, color = 'g',alpha = 1)
    ax.text(0.1,0.36,'Charging time form: ' + time_text, 
    horizontalalignment='left', verticalalignment='top', fontsize = 10, color = 'g',alpha = 1)
    ...
    if type:
        ...
        ax.text(0.06,0.30,'              battery: ' + text, 
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1)  
    ...
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1)
        ...
        ax.text(0.41,0.29,'Effeciency: ' + effeciency+ '%', 
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1) 
    ...
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1)
        ...
        ax.text(0.06,0.18,'              CP:        '+ text , 
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1)
        
        ax.text(0.003,0.24,'Fast charging', 
    horizontalalignment='left', verticalalignment='top', fontsize = 12, alpha = 1, color ='b')
    ...
    if type == 100:        
        ...
        li = df['batteryCurrent']/1000000
        text,w_out = calculate_power(lt,lv,li,df)
        ax.text(0.06,0.09,'              battery: ' + text, 
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1)   
    ...
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1)
        ...
        ax.text(0.41, 0.075,'Effeciency: ' + effeciency + '%', 
    horizontalalignment='left', verticalalignment='top', fontsize = 10, alpha = 1) 
    
        ax.text(0.003,0.065,'Full charging', 
    horizontalalignment='left', verticalalignment='top', fontsize = 12, alpha = 1, color = 'b')
...
def use_time(m, set_min, set_max):
    df = m.df
    print(sys._getframe().f_code.co_name,  sys._getframe().f_lineno, set_min, set_max)
...
def use_soc(m,set_soc_min, set_soc_max):
    print(sys._getframe().f_code.co_name,  sys._getframe().f_lineno, set_soc_min, set_soc_max)
    df = m.df
    
    if set_soc_min >= set_soc_max or set_soc_min > df['SOC'].max() or set_soc_max < df['SOC'].min():
...
def pre_process_file(m,type, v1= None, v2 = None):
    print(sys._getframe().f_code.co_name,  sys._getframe().f_lineno, type,v1, v2)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        if sys.argv[2] != 'option':
            sys.exit("the 2th parameter should be 'option', if you want use option function!")
    if len(sys.argv) >= 2:
        st = sys.argv[1]
    ...
    if len(sys.argv) == 2:
        ...
        ax.text(0.4,0.65, 'use: default (soc to 100)\nfull (all log)\n or fast',horizontalalignment='left', verticalalignment='top', fontsize = 7, alpha = 0.5)
