def plot_qc2_status():
    ...
    state = re.findall(
        '\[.*?(\d+\.\d+)\]\sHVDCP2:\sS_OPTI_(.*)\s\-\>\sS_OPTI_(.*)', text)
    t_st = []
    s = []
...
def qc2_plot():
    ...
    #--a.------V_bus---adj-------------------
    res = re.findall('\[.*?(\d+\.\d+)\]\sHVDCP2:\sVoltage.*?voltage=(\d+)',
                     text)
    lt = []
    lv = []
    for t, v in res:
    ...
    res = re.findall('\[.*?(\d+\.\d+)\]\s.*?state_props\.die_health\s=\s(.*)',
                     text)
    lt = []
    lt = []
    lh = []
...
def get_pl_disable_log(dir):
    ...
    if 1:
        ...
        for line in f.readlines():
            ...
            #--stepper_work---------------------
            r_stepper = re.findall(
                '\[.*?(\d+.\d+)\]\sQCOM-BATT:\sfcc_stepper_work:\sRescheduling\sFCC_STEPPER\swork',
                line)
            if r_stepper != []:
            ...
            if start == 1:
                ...
                class2_ilim = re.findall(
                    'CP_ILIM:\sFCC_VOTER,0\ssame\svote\son\sof\sval=(\d+)',
                    line)
                if class2_ilim != []:
                ...
                class4_ilim = re.findall(
                    'CP_ILIM:\seffective\svote\sis\snow\s(\d+)', line)
                if class4_ilim != []:
                    effec = 1
...
#------pl_notify----------------------
def pl_notify():
    r_pl_notify_c4 = re.findall(
        '\[.*?(\d+.\d+)\]\sPL_ENABLE_INDIRECT:\seffective\svote\sis\snow\s1\svoted\sby\sUSBIN_I_VOTER,0',
        text)
    ...
    r_pl_notify_c2 = re.findall(
        '\[.*?(\d+.\d+)\]\sPL_ENABLE_INDIRECT:\sUSBIN_I_VOTER,0\sIgnoring\ssimilar\svote\s',
        text)
...
def get_cp_irq_data():
    ...
    off_w_0 = re.findall(
        '\[\s*?(\d*?.\d*?)\].*?switcher-off-window\sIRQ\striggered', text)
    if off_w_0 != []:
        off_w_0 = list(map(float, off_w_0))

    # sw is disable
    uv_ov_1 = re.findall(
        '\[\s*?(\d*?.\d*?)\].*?switcher-off-fault\sIRQ\striggered', text)
    if uv_ov_1 != []:
    ...
    TREV_3 = re.findall('\[\s*?(\d*?.\d*?)\].*?irev-fault\sIRQ\striggered',
                        text)
    if TREV_3 != []:
        TREV_3 = list(map(float, TREV_3))

    VPH_OV_HARD_4 = re.findall(
        '\[\s*?(\d*?.\d*?)\].*?vph-ov-hard\sIRQ\striggered', text)
    if VPH_OV_HARD_4 != []:
        VPH_OV_HARD_4 = list(map(float, VPH_OV_HARD_4))

    VPH_OV_SOFT_5 = re.findall(
        '\[\s*?(\d*?.\d*?)\].*?vph-ov-soft\sIRQ\striggered', text)
    if VPH_OV_SOFT_5 != []:
    ...
    TEMP_ALARM_7 = re.findall(
        '\[\s*?(\d*?.\d*?)\].*?temp-alarm\sIRQ\striggered', text)
    if TEMP_ALARM_7 != []:
...
def plot_cp_status(is_plot=1):
    ...
    sd11 = {
        ...
        'S_VIN_REQ': h + j * 4,
        'S_CP_HOT': h + j * 5,
        'S_CP_COOLING': h + 0.3 * 6,
        'S_BALANCED': h + j * 7,
        'S_MAIN_COOLING': h + j * 8,
        'S_MAIN_HOT': h + j * 9,
        'S_CP_IRQ': h + j * 10,
        'S_THERMAL': h + j * 11
    }
...
class Buttonprocess(object):
    ...
    def update(self):
        ...
        #lines = [l8,l9]
        rax = plt.axes([
            0.02, 0.9 - 0.03 * 16 - 0.03 * len(lines), 0.07, 0.03 * len(lines)
        ])
...
if __name__ == '__main__':
    ...
    plt.subplots_adjust(top=0.99, bottom=0.05, left=0.02,
                        right=0.99)  #left = 0.02
    ...
    if ctype == 'USB_PD':
        try:
            ...
            rax = plt.axes([
                0.02, 0.9 - 0.03 * 16 - 0.03 * len(glines), 0.07,
                0.03 * len(glines)
            ])
        except:
        ...
        rax = plt.axes([
            0.02, 0.9 - 0.03 * 16 - 0.03 * len(glines) - 0.1, 0.07,
            0.03 * len(glines)
        ])
