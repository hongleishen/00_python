		...
		struct imi_buf_node ** buf)
{
	...
	if (!list_empty(head)) {
		...
		if (!found) {
			log_e("%x irq, dma addr %x not on list\n", path, addr);
			return -EFAULT;
		}
	}
	log_v("path %d node %p phys %x %x\n", path, bn, bn->buf_addr, sz);
...
int dpu_hal_buffer_ops(int path, void *priv, int this_user, int next_user, int next_module)
{
	...
	if (!list_empty(head)) {
		...
		if (!imi_send_buffer(next_module, (void*)bn,
			...
			log_e("send buf %p to target %x failed\n", bn, next_user);
		}
		FRTOS_mutex_lock(ctrl->dpu_mutex);
		list_del(&bn->usrlist);
		bn = 0;
		...
		log_e("%x irq ,can not find buffer on list\n", path);
		return -EFAULT;
	}
	ret |= bm_buf_get(sz, &bn);
	FRTOS_mutex_lock(ctrl->dpu_mutex);
...
int dpu_op_cmd_parse(struct imi_ipc_msg *msg, struct dpu_control *ctrl)
{
	...
	switch(idm->opcmd) {
		...
		case DPU_CMD_UPDATE_TRIGGER_PARA:
			...
			log_e("DPU_CMD_UPDATE_TRIGGER_PARA\n");
			break;
		...
		case DPU_CMD_GEO_DONE:
			ret = 0;
			log_e("DPU_CMD_GEO_DONE\n");
			break;
		case DPU_CMD_BUFFER_IN:
			ret = 0;
			log_e("DPU_CMD_BUFFER_IN\n");
			break;
		default:
			log_e("%s cmd %d invalid\n", __func__, idm->opcmd);
