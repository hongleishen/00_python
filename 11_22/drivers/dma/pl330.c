...
#define msecs_to_loops(t) (configCPU_CLOCK_HZ / configTICK_RATE_HZ * t)
...
updt_exit:
	...
	if (pl330->dmac_tbd.reset_dmac
		...
		ret = 1;
		pl330_dotask((unsigned long)pl330);
	}
...
static int dmac_alloc_threads(struct pl330_dmac *pl330)
{
	...
	pl330->channels = pvPortMalloc((1 + chans) * sizeof(*thrd));
	if (!pl330->channels)
		return -ENOMEM;
	memset(pl330->channels, 0, (1 + chans) * sizeof(*thrd));
...
static int dmac_alloc_resources(struct pl330_dmac *pl330)
{
	 ...
	 */
	pl330->mcode_cpu = (void *)dma_buf_alloc(chans * pl330->mcbufsz);
	pl330->mcode_bus = (dma_addr_t)pl330->mcode_cpu;
	if (!pl330->mcode_cpu) {
			...
			struct dma_slave_config *slave_config)
{
	...
	if (slave_config->direction == DMA_MEM_TO_DEV) {
		...
		if (slave_config->dst_addr_width)
			pch->burst_sz = ffs(slave_config->dst_addr_width);
		...
		if (slave_config->src_addr_width)
			pch->burst_sz = ffs(slave_config->src_addr_width);
...
static int add_desc(struct pl330_dmac *pl330, gfp_t flg, int count)
{
	...
	desc = pvPortMalloc(count * sizeof(*desc));
	if (!desc)
		return 0;
...
//sgl terminated with addr = 0 and len = 0
static struct dma_async_tx_descriptor *
pl330_prep_slave_sg(struct dma_chan *chan, struct dma_tr_param *sgl,
		...
		unsigned long flg, void *context)
{
	...
	for (i = 0; sgl[i].addr != 0; i++) {
		...
		if (direction == DMA_MEM_TO_DEV) {
			desc->rqcfg.src_inc = 1;
			desc->rqcfg.dst_inc = 0;
			fill_px(&desc->px, pch->fifo_dma, sgl[i].addr,
				sgl[i].len);
		} else {
			desc->rqcfg.src_inc = 0;
			desc->rqcfg.dst_inc = 1;
			fill_px(&desc->px, sgl[i].addr, pch->fifo_dma,
				sgl[i].len);
		}
		...
		desc->rqcfg.brst_len = 1;
		desc->rqtype = direction;
		desc->bytes_requested = sgl[i].len;
	}
...
static void pl330_get_pid(struct pl330_config *dev)
{
	u32 pid, cid, i;
		/*
	 ...
	 */
	for (pid = 0, i = 0; i < 4; i++)
		pid |= (readl(AXI_DMAC_BASE_ADDR + 0x1000 - 0x20 + 4 * i) & 255) <<
			(i * 8);

	for (cid = 0, i = 0; i < 4; i++)
		cid |= (readl(AXI_DMAC_BASE_ADDR + 0x1000 - 0x10 + 4 * i) & 255) <<
			(i * 8);
...
static void pl330_init(void)
{
	...
	pl330 = pvPortMalloc(sizeof(*pl330));
	if (!pl330)
		return;
	...
	pd = &pl330->ddma;
	pl330->mcbufsz = 0;
	pl330->quirks |= PL330_QUIRK_BROKEN_NO_FLUSHP;
	pl330->base = (void *)AXI_DMAC_BASE_ADDR;

	for (i = 0; i < AMBA_NR_IRQS; i++) {
		irq = i + IRQ_DMA_0;
		ret = request_irq(irq, pl330_irq_handler, "pl330", pl330);
		if (ret)
			goto fail1;
	}
	...
	if (!add_desc(pl330, GFP_KERNEL, NR_DEFAULT_DESC))
		dev_warn(NULL, "unable to allocate desc\n");
	...
	pl330->peripherals = pvPortMalloc(num_chan * sizeof(*pch));
	if (!pl330->peripherals) {
		pr_debug("can not alloc peripherals\n");
		goto fail1;
	}
	...
	if (ret) {
		dev_err(NULL, "unable to register DMAC\n");
		goto fail2;
	}
...
fail2:
	vPortFree(pl330->peripherals);
fail1:
	vPortFree(pl330);
}
POST_CORE_INIT(pl330_init);
