...
static int ss_2280_valid_sync(void *ctrl, int chan, int highlow)
{
	struct snr_sync_hw *hw = (struct snr_sync_hw *)ctrl;
	switch (hw->tm.trig_mode) {
		case TRIG_MODE_AUTO:
			write32(hw->base + 0x18, !!highlow);
			return 0;
		...
		case TRIG_MODE_SW_SEPERATE:
			if (chan & (1 << TRIG_CHAN_IR)) {
				gpio_set_value(hw->ss0_gpio, !!highlow);
			}
			if (chan & (1 << TRIG_CHAN_FLOOD)) {
				gpio_set_value(hw->ss1_gpio, !!highlow);
			}
			if (chan & (1 << TRIG_CHAN_DOT)) {
				gpio_set_value(hw->ss2_gpio, !!highlow);
			}
			break;
	}
	return 0;
...
static int ss_2280_chan_mode(void *ctrl, int chan, int mode)
{
	...
	//bit1 set global/ext mode
	write32(hw->base + 0x14, mode << 1);
	if (mode > TRIG_MODE_AUTO) {
		pinmux_set_mode(hw->ext_gpio, 0);
	}
...
void ss_2280_probe(void)
{
	...
	sshw->ss1_gpio = GPIO5;
	sshw->ss2_gpio = GPIO6;
	sshw->ext_gpio = SS_EXT_TRIG_GPIO;
