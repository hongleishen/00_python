...
char *lfs_file_gets(lfs_t *lfs, lfs_file_t *file,
        char *dst_buf, int max_len)
{
    int err = LFS_LOCK(lfs->cfg);
    if (err) {
        return NULL;
    ...
    LFS_TRACE("lfs_file_gets(%p, %p, %p, %"PRIu32")",
            (void*)lfs, (void*)file, dst_buf, size);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    while (n < max_len - 1) {
        res = lfs_file_read(lfs, file, &c, 1);
        if (res <= 0)
            break;
        dst_buf[n++] = c;
        if (c == '\n')
    ...
    LFS_TRACE("lfs_file_gets -> %"PRId32, res);

exit:
    LFS_UNLOCK(lfs->cfg);
    return (n || max_len == 1) ? dst_buf : NULL;
}

int lfs_file_eof(lfs_t *lfs, lfs_file_t *file)
{
    int err = LFS_LOCK(lfs->cfg);
    if (err) {
        return err;
    }
    LFS_TRACE("lfs_file_eof(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    int eof = file->pos >= file->ctz.size;

    LFS_TRACE("lfs_file_eof -> %"PRId32, res);
    LFS_UNLOCK(lfs->cfg);
    return eof;
}
