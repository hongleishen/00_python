        ...
        for item in day:  # item 每个日常记录
            ...
            if data_flag:   # 第一个 item 是日期
                ...
                if len(item) == 2:
                        new_item.append('日常')
                        new_item.append(item[0])
                        new_item.append(float(item[1]))
                        new_item.append('农商')
                
                # ==3 :
                # 1. 虚拟账户 default
                # 2. 账户 default
                # 3. 账户 对账 金额 vcheck      平安 对账 200
                elif len(item) == 3:
                    if not (item[0] in lvirt):     
                        if item[1] == '对账':       # 平安 对账 200
                            new_item.append('item[0]')
                            new_item.append(item[1])
                            new_item.append(float(item[2]))
                            new_item.append('vcheck')
                        else:                        # default 电池 2 平安
                            new_item.append('日常')
                            new_item.append(item[0])
                            new_item.append(float(item[1]))
                            new_item.append(item[2])
                    else:                           # 额外 机票 450  default
                        if item[0] != 'income':
                            new_item.append(item[0])
                            new_item.append(item[1])
                            new_item.append(float(item[2]))
                ...
                elif len(item) == 4:               # income 工资 18000 招商
                    new_item.append(item[0])
                    new_item.append(item[1])
                    new_item.append(float(item[2]))
                    new_item.append(item[3])
                ...
                '''
                # new_item[0] 通常是 虚拟账户   代表消费或收入
                # 如果是自己账户                1. 代表账户间转账  2. 出借
                ...
                # 1.        v -> a
                if new_item[0] in lvirt:        # 虚拟账户  to   账户
