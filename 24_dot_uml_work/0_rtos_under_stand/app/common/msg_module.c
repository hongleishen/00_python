...
int imi_send_event(enum imi_ipc_module target, int evt_id, int *para)
{
	...
	//sending out event;
	LOGV("targeting :%s, evt %d/0x%x", ipc_get_module_str(target), evt_id, evt_id);
	imi_ipc_send_event(1 << target);
	spin_unlock(lock);
	return ret;
}

int imi_send_event_isr(enum imi_ipc_module target, int evt_id, int *para)
{
	struct imi_ipc_msg msg;
	static int seq = 0;
	int ret = 0;
	...
	LOGV("target :%s", ipc_get_module_str(target));
	LOGV("send %d, need %d, evt %d\n", sent, need_resch, evt_id);
	if (sent == pdPASS) {
			...
			struct imi_buf_node* node)
{
	...
	if (!already_in) {
		LOGV("incoming new buffer node %p addr:%x\n", mbuffer->buf,
